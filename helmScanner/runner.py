...
def scan_files():
    ...
    for repoCount in crawlDict:
        ...
        for chartPackage in crawlDict[repoCount]['repoPackages']:
            ...
            #    continue
            if True:
                print(f"Scanning {repo['repoName']}/{chartPackage['name']}| Download Source ")
                ...
                try:
                    ...
                    for filename in glob.glob(f"{downloadPath}/**.tgz", recursive=False):
                        try: 
                            ...
                            print(f"Scanning {repo['repoName']}/{chartPackage['name']}| Extract Source ")
                            os.remove(filename)
                        except:
                            print(f"Failed to extract {repo['repoName']}/{chartPackage['name']}")
                            extract_failures.append([f"{repo['repoName']}/{chartPackage['name']}"])
                            continue
                except:
                    print(f"Failed to download {repo['repoName']}/{chartPackage['name']}")
                    download_failures.append([f"{repo['repoName']}/{chartPackage['name']}"])
                    continue

                runner = helm_runner()

                print(f"SCAN OF {repo['repoName']}/{chartPackage['name']} | Processing Chart Deps")
                ...
                if e:
                    if "Warning: Dependencies" in str(e, 'utf-8'):
                        print(f"V1 API chart without Chart.yaml dependancies. Skipping chart dependancy list for {chartPackage['name']} at dir: {downloadPath}/{chartPackage['name']}. Error details: {str(e, 'utf-8')}")
                    else: 
                        print(f"Error processing helm dependancies for {chartPackage['name']} at source dir: {downloadPath}/{chartPackage['name']}. Error details: {str(e, 'utf-8')}")
    ...
    #### GRAPH. INITIAL NODE AND DEPS   
                if chart_deps:
                    ...
                    for key in chart_deps:
                        print(f" GRAPHING DEPS FOR {chart_deps[key]}")
                        ...
                        if repo['repoName'] in list(current_dep.values())[2]:
                            print(f"{depCombinedRepoName} (repo name in dep url): is probably a local repo dep for {repoChartPathName} Coloring blue")
                            nodecolor = "blue"
                        elif "http" not in list(current_dep.values())[2]:
                             print(f"{depCombinedRepoName} (no url, local files): is probably a local repo dep for {repoChartPathName} leaving black")
                        else:
                            print(f"{depCombinedRepoName} is probably a remote repo dep for {repoChartPathName} Coloring red")
                            nodecolor = "red"
                ...
                results_scan = object
                try:
                    print(f"SCAN OF {repo['repoName']}/{chartPackage['name']} | Running Checkov")
                ...
                try:
                    res = results_scan.get_dict()
                    print(f"SCAN OF {repo['repoName']}/{chartPackage['name']} | Processing Helm Dependancies")
                    #{'common': {'chart_name': 'common', 'chart_version': '0.0.5', 'chart_repo': 'https://charts.adfinis.com', 'chart_status': 'unpacked'}}
                    if chart_deps:
                        for key in chart_deps:
                            print(f" HELMDEP FOUND! {chart_deps[key]}")
                            current_dep = chart_deps[key]
                    ...
                    print(f"CURRENT HELMDEPS LIST {helmdeps_lst}")
        ...
        print(f"Global deps usage: {globalDepsUsage}")
        print(f"Global deps list {globalDepsList}")
...
def run():
    ...
    #result_writer.print_summary_csv(summary_table, checks_table, empty_resources, RESULTS_PATH)
